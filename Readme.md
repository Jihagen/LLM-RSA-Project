...
| H5 | Does a fixed readout change as progressively longer context-conflict prefixes arrive? | Exploratory; stimulus audit passes, rerun pending |
...
| H5 | **Design complete; rerun pending.** The obsolete cross-token output cannot show revision. The fixed-sentinel incremental runner now has balanced directions, matched controls for all 98 items, isolated-resolver baselines, and a passing structural audit. `light` is a documented cross-POS exclusion. No claim of human-validated garden-path processing is made. |
...
- Run the now-complete fixed-sentinel H5 design. Directions and matched controls are
